...
def pray(q, uuid, flashcards, openended, mcq, summary):
    ...
    with ProcessPool(max_workers=4) as pool:
        ...
        if openended:
            future.put(
                (pool.schedule(gpt.generateQuiz, args=(q,)), ["openendedArray", uuid])
            )
        if mcq:
        ...
        if flashcards:
            future.put(
                (
                    pool.schedule(gpt.generateFlashcards, args=(q,)),
                    ["flashcardsArray", uuid],
                )
            )
        if summary:
            future.put(
                (pool.schedule(gpt.generateSummary, args=(q,)), ["summaryResult", uuid])
            )
