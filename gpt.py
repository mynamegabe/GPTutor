...
def askGPT(gpt_session_token, prompt, conversation_id=None, parent_id=None):  # You can start a custom conversation
...
def generateQuiz(gpt_session_token, content):
    ...
    prompt = "Generate 5 quiz questions and their respective answers for the following content:\n " + content + instruction
    ...
    if len(questions) == len(answers):
        ...
        try:
            for question in strippedResponse[::2]:
                ...
                answers.append(
                    strippedResponse[strippedResponse.index(question)+1].split(':')[1].strip())
        except:
...
def generateMCQ(gpt_session_token, content):
    ...
    prompt = "Generate 5 Multiple-Choice quiz questions and their respective answers for the following content:\n " + content + instruction
    rsp = askGPT(gpt_session_token, prompt)
    ## Parse the response
    # Split into sections
    print(rsp)
    ...
    if len(sections) == 2:
        ...
        print("Assuming Format is of form: \n1. Question\nA) Option 1\nB) Option 2\nC) Option 3\nD) Option 4\nAns: A")
        lines = rsp.split('\n')  
        ...
        for question in lines[::6]:
            ...
            print(answers)
            return [[questions[i], possible_answers[i], answers[i]] for i in range(len(answers))]
